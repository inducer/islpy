...
def dim_type_reduce(v: _isl.dim_type):
    return (_isl.dim_type, (int(v),))
...
def space_get_id_dict(
        ...
        ) -> dict[_isl.Id, tuple[_isl.dim_type, int]]:
...
def space_create_from_names(ctx: _isl.Context, set=None, in_=None, out=None, params=()):
...
def obj_or(self: SelfT, other: object) -> SelfT:
    try:
...
def obj_and(self: SelfT, other: object) -> SelfT:
    try:
...
def obj_sub(self: SelfT, other: object) -> SelfT:
    try:
...
def map_get_basic_maps(self: _isl.Map):
...
@_memoize_on_first_arg
def obj_get_var_names(self, dimtype: _isl.dim_type) -> list[str]:
    """Return a list of dim names (in order) for :class:`dim_type` *dimtype*."""
    return [self.get_dim_name(dimtype, i) for i in range(self.dim(dimtype))]


def pwaff_get_pieces(self) -> Sequence[tuple[_isl.Set, _isl.Aff]]:
    result = []
